...
def downsample(lidar_np, num_samples=500):
    sampler = Random_Sampler(num_samples)
    sparse_depth = sampler.sample(lidar_np)
    return sparse_depth
    ...
    if args.lr_policy == 'lambda':
        ...
        class WarmupLR(lr_scheduler._LRScheduler):
            ...
            def get_lr(self):
                ...
                elif self.last_epoch >= self.warmup_steps and self.last_epoch < self.warmup_steps + 5:
                    return [base_lr for base_lr in self.base_lrs]
                else:
                    return [base_lr * math.exp(-(self.last_epoch - self.warmup_steps + 6)//5) for base_lr in self.base_lrs]
        
        scheduler = WarmupLR(optimizer, warmup_steps=5)    
    elif args.lr_policy == 'none':
        scheduler = None
    else:
        return NotImplem0entedError('learning rate policy [%s] is not implemented', args.lr_policy)
    return scheduler
