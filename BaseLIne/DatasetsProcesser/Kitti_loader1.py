...
class Random_Sampler():
    ...
    def sample(self, depth):
        mask_keep = depth > 0
        n_keep = np.count_nonzero(mask_keep)#找出非0深度点个数

        if n_keep == 0:
            return mask_keep
        else:
            depth_sampled = np.zeros(depth.shape)#同大小0矩阵
            prob = float(self.num_samples) / n_keep#
            mask_keep =  np.bitwise_and(mask_keep, np.random.uniform(0, 1, depth.shape) < prob)#按位与计算（）
            depth_sampled[mask_keep] = depth[mask_keep]
            return depth_sampled


class Kitti_preprocessing(object):#加载数据所在地址及参数到列表中
    def __init__(self, dataset_path, input_type='depth', side_selection=''):
        self.train_paths = {'img': [], 'lidar_in': [], 'gt': [] ,'ip_gt':[]}
            ...
            for root, dirs, files in os.walk(os.path.join(self.dataset_path, type_set)):
                if re.search(self.depth_keyword, root):
                    ...
                    self.val_paths['gt'].extend(sorted([os.path.join(root, file) for file in files
                                                        ...
                                                        and re.search('val', root)
                                                        and re.search(self.side_selection, root)]))
                if re.search(self.ip_keyword, root):
                    self.train_paths['ip_gt'].extend(sorted([os.path.join(root, file) for file in files
                                                        if re.search('ip_processed', root)
                                                        and re.search('train', root)
                                                        and re.search(self.side_selection, root)]))
    ...
    def prepare_dataset(self):##获取数据集文件的字典
        ...
        if self.use_rgb:
            ...
            print(len(self.train_paths['ip_gt']))
