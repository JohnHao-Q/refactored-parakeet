...
class New_net(nn.Module):
    ...
    def __init__(self, max_depth):
        ...
        self.eca_128 = eca_layer(channel=256, k_size=3)
        self.eca_64 = eca_layer(channel=128, k_size=3)
        self.eca_32 = eca_layer(channel=64, k_size=3)
        self.eca_16 = eca_layer(channel=16, k_size=3)
        
        self.joint_conv1 = nn.Sequential(ConvBN(256, 128, 3, 1, 1, 1),nn.ReLU(), nn.Conv2d(128,128,kernel_size=1,padding=0,stride=1,bias=True),nn.ReLU())#128/8        
        
        #self.joint_conv1_1 = nn.Sequential(ConvBN(192, 192, 3, 1, 1, 1),nn.ReLU(), nn.Conv2d(192,128,kernel_size=1,padding=0,stride=1,bias=True),nn.ReLU())#128/8
        
        #self.joint_conv1_2 = nn.Sequential(ConvBN(128, 128, 3, 1, 1, 1),nn.ReLU(), nn.Conv2d(128,128,kernel_size=1,padding=0,stride=1,bias=True))#128/8
        
        self.deconv1 = Deconv(128, 64)#上采样64/4
        
        self.joint_conv2 = nn.Sequential(ConvBN(128,64,3,1,1,1),nn.ReLU(),nn.Conv2d(64,64,kernel_size=3,padding=1,stride=1,bias=True))
        
        self.deconv2 = Deconv(64, 32)
        
        self.joint_conv3 = nn.Sequential(ConvBN(64,32,3,1,1,1),nn.ReLU(),nn.Conv2d(32,32,kernel_size=3,padding=1,stride=1,bias=True))
        
        self.deconv3 = Deconv(32, 16)
        
        self.joint_conv4 = nn.Sequential(ConvBN(32,16,3,1,1,1),nn.ReLU(),nn.Conv2d(16,16,kernel_size=3,padding=1,stride=1,bias=True))
    ...
    def forward(self, x_in):
        ...
        d_128, d_64, d_32, d_16 = self.depth_extact(d_in)
        
        fuse_map_128 = self.eca_128(torch.cat((map_128, d_128), dim=1))#256/8
        joint_feature1 = self.joint_conv1(fuse_map_128)#128/8
        deconv_1 = self.deconv1(joint_feature1)#64/4
        
        fuse_map_64 = self.eca_64(torch.cat((map_64, d_64), dim=1)) 
        joint_feature2 = self.joint_conv2(fuse_map_64)#64/4
        deconv_2 = self.deconv2(joint_feature2 + deconv_1)#32/2
        
        fuse_map_32 = self.eca_32(torch.cat((map_32, d_32), dim=1))
        joint_feature3 = self.joint_conv3(fuse_map_32)#32/2
        deconv_3 = self.deconv3(joint_feature3 + deconv_2)#16/1
        
        fuse_map_16 = self.eca_16(torch.cat((map_16, d_16), dim=1))
        joint_feature4 = self.joint_conv4(fuse_map_16)#16/1        
